    ...
    <Box>
      ...
      <Box marginY={4}>
        <Grid container spacing={2}>
          {userRatings ? userRatings.map((item, i) => (
      ...
      <Box marginTop={2}>
        ...
        <Box>
          <FormControlLabel
            control={<Checkbox checked={owned} onChange={(e) => setOwned(e.target.checked)} color="primary" />}
            label="Do you own this game?"
          />
        </Box>
        <Stack marginTop={2} spacing={1} direction={'row'}>
          {localReviewData && localReviewData.length > 0 && rating ? (
